...
def borrow_book(request, book_id, reader_id):  # 借書
    ...
    Book.objects.filter(id=book_id).update(
        update_at=current_datetime, status=1)
    Reader.objects.filter(id=reader_id).update(
        update_at=current_datetime, borrow_times=reader.borrow_times+1)
...
def renew_book(request, book_id, reader_id):  # 續借
    try:
        borrow = Borrow.objects.get(
            book_id=book_id, reader_id=reader_id, status=0)
    except ObjectDoesNotExist:
        return LibraryError.to_json_response(
            LibraryError.INVALID_PARAMETER,
    ...
    current_datetime = get_current_datetime()
    Borrow.objects.filter(id=borrow.id).update(
        update_at=current_datetime, times=borrow.times+1)
...
def return_book(request, book_id, reader_id):  # 還書
    ...
    if borrow.borrow_at + timedelta(days=borrow.times * BORROW_DAYS) < timezone.now():  # 逾期還書
        ...
        Reader.objects.filter(id=reader_id).update(
            update_at=current_datetime, violation_times=reader.violation_times + 1)

    Borrow.objects.filter(id=borrow.id).update(
        update_at=current_datetime, return_at=current_datetime, status=9)
